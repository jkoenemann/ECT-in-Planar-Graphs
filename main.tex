...
% starting the proof with edge max counterex
 ...
 Let $u_1,u_2,..,u_l$ be the neighbours of $v$ in clockwise order and let $u_{l+1}=u_1$. We claim that $u_i,u_{i+1}$ cannot both have degrees less than 6. For the sake of contradiction assume that this is not true, then $ u_i,v,u_{i+1} $ are contained in a single face $f$, and $u_i,u_{i+1}$ are not neighbours. Let $w$ be the other neighbour of $u_i$ in $f$; since $G$ contains no adjacent light neighbours, $w$ is heavy. So adding an edge between $v,w$ inside $f$ would not create adjacent vertices whose degrees sum to less than 11 nor create a face of length 2, which contradicts our choice of $G$.  Thus $v$ does not have 2 consecutive light neighbours, and so has at most as many light neighbours as heavy neighbours.  Let $l$ and $h$ be the number of light and heavy neighbours of $v$. We consider 3 cases.
 
 Suppose $v$ has a neighbour $u$ of degree 3. Vertex $v$ starts with charge at most $6-l-h$ and it receives charge at most 1 from each light neighbour and none from heavy neighbours. So the charge of $v$ is at most  $6-l-h +l =6 -h $.  Thus v has at most 5 heavy neighbours and hence at most 10 neighbours. Since $v$ is adjacent to $u$ which has degree 3, this proves the lemma. 
...
whose degrees sum to $13$ in such a graph $G$, and suppose, w.l.o.g., that $v$ has degree no smaller than that
...
has a neighbour $w$ on $f$ different from $v$ and $u'$. This node $w$ must be heavy, and hence we would be
able to add edge $vw$ to $G$ without violating planarity, which is a contradiction to $G$'s maximality. This shows that
...
\begin{proof}
  ...
  In Figure
  \ref{fig:simplepd}.(i), white nodes have degree two in $G[V\setminus
  ...
  Now suppose that $v \in \bar{X} \cap C_1$ is a node of
...
\begin{definition}
Let us call a face $f$ of $G_2$ \emph{particular}, if  $f$ is adjacent to exactly 5 faces of degree 5 or less and 5 faces of degree 6 or more, and they alternate in the clockwise order around $f$. We  also require that at least 4 of the faces of length 5 or less have length 3. Further all vertices on $f$ have degree 3.
\end{definition}
...
1) $u$ has degree 3 and if $v$ has degree more than 6 it has degree at most $2a_1 + a_2 + 2c_1+c_2 +d $ with $ 6-(a_1+a_2) -1.5(c_1+c_2) -d -b > 0 $. Here $ a_1 $ is the number of neighbours of degree 3 for which  the next neighbour of $v$  in the clockwise order is heavy and $a_2$ is the number of other neighbours of degree 3.  %  of $G$ that contain $vw$ are triangles, $a_2$ is the number of neighbours $w$ of $v$ of degree 3 for which not both faces of $G$ that contain $uw$ are triangles 
$c_1$ is the number of neighbours of degree 4 or 5 for which the next neighbour of $v$ in the clockwise order is heavy, and $c_2$ is the number of other neighbours of degree 4 or 5. $b$ is the number of faces of length 4 or more containing $v$ and both a light and heavy neighbour of $v$ in $G$. $d$ is the number of heavy neighbours $w$ of $v$ whose such that the next clockwise neighbour of $v$ is also heavy; see Figure \ref{Triangulating}. \\ % $b$ is the number of faces of  
...
\end{lemma}
When  $G$ and $v \in V(G)$ are specified we will denote by $a'_1,a'_2,b',d',c'_1,c'_2$ neighbours as those neighbours of $v$ counted by $a_1,a_2,b,d,c_1,c_2$ respectively and a $a'_1,a'_2,b',d',c'_1,c'_2$  edge as an edge between $v$ and a $a'_1,a'_2,b',d',c'_1,c'_2$ neighbour respectively. We also say that a face of $G^*$ is an $a'_1,a'_2,b',d',c'_1,c'_2$ face if the face corresponds to an $a'_1,a'_2,b',d',c'_1,c'_2$ vertex of $G$ respectively.
\begin{proof}
%(of part not in \cite{JV13})
%proof up to a reasonable length
 If $G$ contains  2 adjacent light neighbours, the proof is clear. Otherwise note that each heavy neighbour of $v$ either follows an $a_1$ neighbour, a $c_1$ neighbour, or another heavy edge and are counted by $a_1,c_1,d$ respectively. Thus $v$ has degree at most $2a_1 + a_2 + 2c_1+c_2 +d $. Let $G'$ be a triangulation of $G$ by drawing edges between heavy vertices. Note that this is possible, since any face of length four or more contains 2 heavy nodes, and we can add an edge between them.  Just as in \cite{JV13}, by maximality of our triangulation $v$ cannot have 2 consecutive light neighbours in $G'$.
...
Since each light edge is preceded and followed by a heavy edge hence $ |\delta_{G'}(v) \backslash E' \backslash E'' | \geq  2a_1 +2a_2+2(c_1+c_2)$. Calculating charge thus gives us $ 6-(a_1+a_2) -1.5c -d -b > 0 $.
\begin{figure}[h]
\begin{center}
...
In case 1) of lemma \ref{light+} we have that $v$ has degree at most $ 10 - a_2 -c_2 -2 \lfloor 0.5(c_1+c_2) \rfloor -d -2b$
\end{remark}
\begin{proof}  Using same variables as Lemma \ref{light+}, we get $ 6 > (a_1+a_2) +1.5(c_1+c_2) +d +b $ so $ 5 \geq \lfloor (a_1+a_2) +1.5(c_1+c_2) +d +b \rfloor $ so  $ 5-(a_1+a_2) - \lfloor 1.5(c_1+c_2) \rfloor -d -b \geq 0$ which implies $  10 \geq 2(a_1+a_2) +2(c_1+c_2) +2 \lfloor 0.5(c_1+c_2) \rfloor +2d +2b \geq \text{deg}(v)  +a_2 +c_2 +2 \lfloor 0.5c \rfloor +d +2b  $.
\end{proof}
%
\begin{lemma}\label{even reduced+}
We can find a cycle $C$ of $G_2$, even in $G$ such that either: \\
1)  $C$ is the union of 2 faces $f_1$ and $f_2$ of $G_2$ with, $f_1$ length 3 and at most  $ \text{max}(9, 11- a_2-c_2 -2 \lfloor 0.5 (c_1+c_2) \rfloor -d  -b $) pieces. \\  %Where $ a_2, c_1,c_2 ,d ,b $ are as in Lemma \ref{light+}  for $G_2^*$ with $v=f_1$  and $C$ is disjoint from the interior of any double piece. \\ 
2) $C$ has at most 9 pieces and is disjoint from the interior of any double piece. \\
3) $C$ has at most $10- a_2 -c_2 -2 \lfloor 0.5 (c_1+c_2) \rfloor -d  -b $  pieces. One piece may be double. \\
4) $C$ has at most 7 pieces. \\
Where for 1),3) $a_1,a_2, c, d, b ,c_2$ are as in Lemma \ref{light+} with  $ v=v_2$  $u=f_1$ in the dual of $G_2$.
\end{lemma}
\begin{proof}
%\p proof
 Apply Lemma \ref{light+} to $G_2^*$.  In case 1) of Lemma \ref{light+}  we find a node $u$ of degree 3 adjacent to a node $v$ of degree at most $10-a_2-c_2-2\lfloor 0.5 (c_1+c_2) \rfloor -d -b$  in $G^*$. Here $a_2,c_1,c_2,d,b$ are as in Lemma \ref{light+}. Let $f_1 , f_2$ be the faces corresponding to $v,u$ in G if $f_1$ or $f_2$ is even such an even cycle satisfies the conditions of out lemma. Otherwise, $f_1 \cup f_2$ is an even cycle in G with at most  $11- a_2 -2 \lfloor 0.5 c \rfloor -d  -b $ pieces. Case 2) is similar.
\end{proof}
Our revised algorithm for this part will first look for an even cycle with 9 or fewer edges in the 1-compression. If this cannot be found it will then look for an even cycle satisfying lemma \ref{even reduced+} and return case 4) (at most 7 pieces) if possible. We claim this algorithm is a 9-approx. 
\begin{proof} 
% \p breaking proof into cases and case 1)
Let us consider each case of Lemma \ref{even reduced+}. Define $c:=c_1+c_2$. 

Case 1). If $C$ has 9 or fewer pieces the proof is clear. Otherwise, $b=0, \ \ c \leq 1$, let $v_1, v_2, .., v_l$ be the cycle $C$ in $G_2$, with $v_1, v_2, v_3$ vertices belonging to a triangle face of $G_2$. We denote: \begin{equation}
    ...
    \end{cases}
\end{equation} 
Where $ v_{l+i} = v_i $ in above equation. If $l=11$, then $a_2, c_2, d=0$. So $v_1v_3 ... v_l$ has length at most $2a_1+2c$ and so $a_1 \geq 4$, where $a_1$ is the number of triangle faces sharing an edge with $v_1v_3,..,v_l$. In fact, just as in the 10 approx case $v_1 v_3 ... v_l$ must be particular. 
Let $v_j t_j v_{j+1}, v_k t_k v_{k+1}, v_r t_r v_{r+1}$ be 3 different $a'_1 $ faces of length 3 that have an edge on $v_1v_3 ... v_l$ that are not $v_1 v_2 v_3$. If any 2 distinct $ q,r \in \{ j,k,r \} $ satisfy $|q-r| \neq 3 \ \ \text{mod} l$ then $ p(v_{q-1}, v_q) \cup p(v_q,v_{q+1}) \cup p(v_{q+1},v_{q+2}), p(v_{r-1}, v_r) \cup p(v_r,v_{r+1}) \cup p(v_{r+1},v_{r+2})$ are disjoint and each contain at most 2 hit nodes. The remaining 5 pieces not containing vertices of  $ p(v_{q-1}, v_q) \cup p(v_q,v_{q+1}) \cup p(v_{q+1},v_{q+2}), p(v_{r-1}, v_r) \cup p(v_r,v_{r+1}) \cup p(v_{r+1},v_{r+2})$  contain at most 5 hit nodes. Thus $C$ contains at most 9 hit nodes. 

Finally suppose $l=10$, then $a_2+c_2+d \leq 1$.  The length $l-1$ of $v_1v_3,..,v_l$ is at most $2a_1+a_2+2c+d$ and so $a_1 \geq 3$. At most one of the $a'_1$ neighbours of $v_1v_3,..,v_l$ of $v_1v_3,..,v_l$ in $G^*_2$ follows an $a'_2$ or $c'_2$ node in the clockwise order and at most one of them is $v_1v_2 v_3$ in $G_2$. Let $v_jt v_{j+1}$ be an $a'_1$ face  of $v_1v_3,..,v_l$ in $G_2$ that does not follow an $a'_2$ or $c'_2$ node, then by remark \ref{consecutive}  $p(v_{q-1}, v_q) \cup p(v_q,v_{q+1}) \cup p(v_{q+1},v_{q+2})$ contains at most 2 hit nodes and the remaining 7 pieces of $C$ not including nodes of $p(v_{q-1}, v_q) \cup p(v_q,v_{q+1}) \cup p(v_{q+1},v_{q+2})$ contain at most 7 hit nodes. So $C$ has at most 9 hit nodes.
% note we have not proven that an a_1 neighbour cannot follow a a neighbour may have to change the proof for that.
%\p case 1a
...
\end{figure}
%\p case 3a)
Case 3a). We have $b = \lfloor 0.5c \rfloor = d = a_2 =0$. Then there are at least 4 $ a'_1  $ edges. Since $b=0$ each endpoint of an $a'_1$ edge $v_i v_{i+1}$ has exactly 3 neighbours. Thus we can find 2 $a'_1$ edges $ v_i v_{i+1} , v_j v_{j+1}$ with $i \neq j-2,j+2$.  Hence $m'(v_{i-1},v_{i+2} )$  , $m'(v_{i-1},v_{i+2} )$ each contain at most 2 hit nodes and hence $C_1$ contains at most 8 hit nodes. 
